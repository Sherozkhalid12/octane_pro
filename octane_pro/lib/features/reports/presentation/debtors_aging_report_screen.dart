...
class DebtorsAgingReportScreen extends ConsumerStatefulWidget {
  const DebtorsAgingReportScreen({super.key});

  @override
  ConsumerState<DebtorsAgingReportScreen> createState() =>
      _DebtorsAgingReportScreenState();
}

class _DebtorsAgingReportScreenState
    extends ConsumerState<DebtorsAgingReportScreen> {
  ...
  Future<void> _loadAgingReport() async {
    ...
    final customers = await _customerRepo.getAllCustomers(activeOnly: false);
    ...
    for (var customer in customers) {
      ...
      final ledger = await _customerRepo.getCustomerLedger(customer.id);
      double current = 0;
      double days30 = 0;
      double days60 = 0;
      double days90 = 0;
      ...
      for (var entry in ledger) {
        ...
        final amount = (entry['debit'] as num?)?.toDouble() ?? 0.0;
  ...
  Future<void> _exportReport() async {
    ...
    await ExportService.instance.showExportDialog(
      ...
      fileName: 'debtors_aging_${DateTime.now().millisecondsSinceEpoch}',
      data: exportData,
      headers: [
        'Customer',
        'Current (0-30)',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Debtors Aging Report'),
        leading: IconButton(
